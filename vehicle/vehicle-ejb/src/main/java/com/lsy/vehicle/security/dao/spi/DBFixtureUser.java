...
public class DBFixtureUser
{
	...
	public void createDefaultDataInDatabase()
	{
		...
		this.createUser("admin", "admin@junit").setRole(Role.ADMIN).createUser("agent", "agent@junit")
				.setRole(Role.AGENT).createUser("customer", "customer@junit").setRole(Role.CUSTOMER).createFleetGroup()
				.addFleet(COMPANY_NAME).addUser(0).addUser(1).addUser(2).persistAll();
	}
